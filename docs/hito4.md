...
A continuación se instalan utilizando el grupo de recursos `CCGroupFC` creado en el [ejercicio 2 del Tema 4](https://github.com/MarAl15/EjerciciosCC/blob/master/Tema4/tema4.md#ejercicio-2):

```console
$ az vm create --name <SO>H4 --image <URN_asociada> --resource-group CCGroupFC --admin-username usuario --size Standard_B1s --ssh-key-value ~/.ssh/id_rsa.pub --public-ip-address-allocation static
```
